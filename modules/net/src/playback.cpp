...
namespace hen
{
    ...
    *  不建议调用该SDK中接口.
    */
    void CALLBACK fPlayESCallBack(LONG play_handle, NET_DVR_PACKET_INFO_EX* pack_info, void* user_data)
    {
        static size_t count = 0;
        static size_t total = 0; // 2656551
        static size_t au_total = 0; // 2656551
        count++;
        ...
        {
            cr_ensure(p.dwPacketSize == 320);
            au_total += p.dwPacketSize;
            fmt::println("#{} A {} len:{}/{}",
                         count,
                         to_string(time),
                         p.dwPacketSize,
    ...
    Playback::Playback(int session, PlaybackInfo info)
    {
        ...
        auto r = NET_DVR_SetPlayBackESCallBack(m_handle, fPlayESCallBack, nullptr);
        hik_ensure(r);
    }
    ...
    void Playback::control(unsigned code)
    {
        ...
        auto r = NET_DVR_PlayBackControl(m_handle, code, 0, nullptr);
        hik_ensure(r);
    }
